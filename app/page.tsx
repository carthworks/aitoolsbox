...
const categories: Category[] = [
  ...
  },
  {
    ...
    tools: [
      ...
      { slug: "latency-checker", title: "Latency Checker", desc: "Compare response times across models or regions.", isPublish: false },
      { slug: "stream-visualizer", title: "Streaming Output Visualizer", desc: "Watch token-by-token generation in real time.", isPublish: false },
  ...
  },
  {
    ...
    color: "bg-rose-200",
    tools: [
      { slug: "prompt-injection", title: "Prompt Injection Detector", desc: "Detect and analyze prompt injection attempts in user inputs.", isPublish: false },
      { slug: "pii-detector", title: "PII Detector", desc: "Identify personal information (emails, SSNs, credit cards) in prompts.", isPublish: false },
      { slug: "content-moderation", title: "Content Moderation Tool", desc: "Check text for harmful, toxic, or inappropriate content.", isPublish: true },
      { slug: "adversarial-prompt", title: "Adversarial Prompt Tester", desc: "Test model robustness against adversarial inputs.", isPublish: false },
      { slug: "ai-content-detector", title: "AI-Generated Content Detector", desc: "Detect if text was likely generated by AI models.", isPublish: false },
      { slug: "data-leakage", title: "Data Leakage Checker", desc: "Scan for potential training data leakage in model outputs.", isPublish: false },
      { slug: "bias-audit", title: "Bias Audit Tool", desc: "Audit model outputs for gender, racial, or cultural bias.", isPublish: false },
    ],
  },
  {
...
const getCategoriesWithExistenceCheck = (): Category[] => {
  ...
  const existingFolders = new Set([
    'about', 'ai-glossary', 'api', 'api-tester', 'asn', 'aws-s3', 'bias-detector', 'cert-parser',
...
export default function HomePage() {
  ...
  const [loadingTool, setLoadingTool] = useState<string | null>(null); // track loading tool
  ...
  // RenderIcon: render icons consistently on server and client
                ...
                {favoritesResolved.length === 0 ? (
                  ...
                  favoritesResolved.slice(0, 6).map((t) => (
                    <div key={t.slug} className="flex items-center justify-between text-sm py-1">
                      <Link href={`/${t.slug}`} onClick={() => handleToolClick(t.slug)} className="hover:underline flex items-center gap-2">
                        {mounted && loadingTool === t.slug && <Loader2 className="w-3 h-3 animate-spin" />}
                        {t.title}
                      </Link>
                  ...
                  {favoritesResolved.length === 0 ? (
                    ...
                    favoritesResolved.slice(0, 6).map((t) => (
                      <Link
                        ...
                        title={t.title}
                      >
                        {mounted && loadingTool === t.slug && <Loader2 className="w-4 h-4 animate-spin text-indigo-600" />}
              ...
              <div className="mt-3">
                ...
                <div className="flex gap-2 overflow-x-auto py-1">
                  {recentResolved.map((t) => (
                    ...
                    >
                      {mounted && loadingTool === t.slug && <Loader2 className="w-3 h-3 animate-spin text-indigo-600" />}
                      <span>{t.title}</span>
                    </Link>
                  ))}
                </div>
          ...
          <section className="grid md:grid-cols-2 gap-6">
            {filteredCategories.map((cat) => (
              <div
                ...
                className={`rounded-lg shadow-sm ${cat.color} border bg-opacity-60 transition-all duration-300 ${openCategories[cat.title] ? 'p-4' : 'p-4 pb-4'}`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="ml-2 flex items-center gap-2">
                    <button
                      ...
                      className="text-xs px-2 py-1 rounded border bg-white hover:bg-slate-50 transition-colors"
                ...
                <div className={`grid gap-3 transition-all duration-300 overflow-hidden ${openCategories[cat.title] ? 'mt-4 opacity-100' : 'mt-0 opacity-0 max-h-0'}`}>
                  {(cat.tools || []).map((t) => {
                            ...
                            <>
                              <div className="flex gap-2">
                                ...
                                <Link
                                  href={`/${t.slug}`}
                                  onClick={() => handleToolClick(t.slug)}
                                  className="px-2 py-1 rounded border text-xs bg-white hover:bg-slate-50 flex items-center gap-1"
                                  title={`Open ${t.title}`}
                                >
                                  {mounted && loadingTool === t.slug && <Loader2 className="w-3 h-3 animate-spin" />}
                                  Open
                              ...
                              <a href={`/${t.slug}`} onClick={() => handleToolClick(t.slug)} className="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1">
                                {mounted && loadingTool === t.slug && <Loader2 className="w-3 h-3 animate-spin" />}
